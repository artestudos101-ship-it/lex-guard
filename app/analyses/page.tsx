...
import { ArrowRight, ChevronLeft, ChevronRight, FileText, MessageSquare, Plus, Sparkles } from "lucide-react"
...
export default function AnalysesPage() {
  ...
  return <AppShell title="Minhas análises" description="Análise + processo + evidência + decisão" action={<Button size="sm" render={<Link href="/analyses/new" />}><Plus data-icon="inline-start" /> Nova análise</Button>}>
    <div className="mx-auto flex max-w-[1550px] flex-col gap-5">
      ...
      <div className={`grid h-[calc(100vh-12.5rem)] min-h-0 overflow-hidden rounded-xl border bg-card shadow-sm ${conversationsOpen && contextOpen ? "lg:grid-cols-[300px_minmax(0,1fr)_300px]" : conversationsOpen ? "lg:grid-cols-[300px_minmax(0,1fr)]" : contextOpen ? "lg:grid-cols-[minmax(0,1fr)_300px]" : "lg:grid-cols-1"}`}>
        {conversationsOpen ? <aside className="flex min-h-0 flex-col border-b lg:border-b-0 lg:border-r"><div className="border-b p-3"><div className="flex items-center gap-2"><div className="min-w-0 flex-1"><p className="text-sm font-semibold">Conversas</p><p className="text-xs text-muted-foreground">{visible.length} análises visíveis</p></div><Sparkles className="size-4 text-primary" /><Button variant="ghost" size="icon-sm" aria-label="Recolher conversas" onClick={() => setConversationsOpen(false)}><ChevronLeft /></Button></div><div className="mt-3"><AnalysisSearch analyses={analyses} value={query} onValueChange={setQuery} /></div></div><ScrollArea className="min-h-0 flex-1"><div className="flex flex-col gap-1 p-2">{visible.map((analysis) => <button key={analysis.id} type="button" onClick={() => setSelectedId(analysis.id)} className={`rounded-lg p-3 text-left transition-colors ${analysis.id === selected?.id ? "bg-primary/10" : "hover:bg-muted/50"}`}><div className="flex items-start gap-3"><div className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-md bg-muted text-primary"><MessageSquare className="size-4" /></div><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{analysis.title}</p><p className="mt-1 truncate text-xs text-muted-foreground">{analysis.orgao}</p><div className="mt-2 flex gap-2"><Badge variant="outline" className="text-[10px]">{analysis.status === "processing" ? `${analysis.progress}%` : analysis.status === "completed" ? "Concluída" : "Na fila"}</Badge><span className="font-mono text-[10px] text-muted-foreground">{analysis.id}</span></div></div></div></button>)}</div></ScrollArea></aside> : <div className="hidden items-start p-2 lg:flex"><Button variant="ghost" size="icon" aria-label="Abrir conversas" onClick={() => setConversationsOpen(true)}><ChevronRight /></Button></div>}
        ...
        {contextOpen ? <aside className="flex min-h-0 flex-col border-t bg-muted/10 lg:border-l lg:border-t-0"><div className="flex items-center gap-2 border-b p-3"><div className="min-w-0 flex-1"><p className="text-sm font-semibold">Contexto</p><p className="text-xs text-muted-foreground">Resumo da decisão</p></div><Button variant="ghost" size="icon-sm" aria-label="Recolher contexto" onClick={() => setContextOpen(false)}><ChevronRight /></Button></div><ScrollArea className="min-h-0 flex-1"><div className="p-3">{selected ? <><DecisionContext analysis={selected} /><Button className="mt-3 w-full" render={<Link href={`/analyses/${selected.id}`} />}><ArrowRight data-icon="inline-end" /> Abrir contexto completo</Button></> : null}</div></ScrollArea></aside> : <div className="hidden items-start justify-end p-2 lg:flex"><Button variant="ghost" size="icon" aria-label="Abrir contexto" onClick={() => setContextOpen(true)}><ChevronLeft /></Button></div>}
      </div>
    </div>
  </AppShell>
}
