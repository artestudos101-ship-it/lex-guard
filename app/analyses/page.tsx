  ...
  return <AppShell title="Minhas análises" action={<Button size="sm" render={<Link href="/analyses/new" />}><Plus data-icon="inline-start" /> Nova análise</Button>}>
    <div className="mx-auto flex max-w-[1550px] flex-col gap-5">
      ...
      <div className={`grid h-[calc(100vh-12.5rem)] min-h-0 overflow-hidden rounded-xl border bg-card shadow-sm ${conversationsOpen && contextOpen ? "lg:grid-cols-[300px_minmax(0,1fr)_300px]" : conversationsOpen ? "lg:grid-cols-[300px_minmax(0,1fr)]" : contextOpen ? "lg:grid-cols-[minmax(0,1fr)_300px]" : "lg:grid-cols-1"}`}>
        ...
        {contextOpen ? <aside className="flex min-h-0 flex-col border-t bg-muted/10 lg:border-l lg:border-t-0"><div className="flex items-center gap-2 border-b p-3"><div className="min-w-0 flex-1"><p className="text-sm font-semibold">Contexto</p><p className="text-xs text-muted-foreground">Resumo da decisão</p></div><Button variant="ghost" size="icon-sm" aria-label="Recolher contexto" onClick={() => setContextOpen(false)}><ChevronRight /></Button></div><ScrollArea className="min-h-0 flex-1"><div className="p-3">{selected ? <DecisionContext analysis={selected} /> : null}</div></ScrollArea></aside> : <div className="hidden items-start justify-end p-2 lg:flex"><Button variant="ghost" size="icon" aria-label="Abrir contexto" onClick={() => setContextOpen(true)}><ChevronLeft /></Button></div>}
      </div>
    </div>
  </AppShell>
}
