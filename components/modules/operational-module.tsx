...
export function OperationalModule({ name }: { name: string }) {
  ...
  return <AppShell title={module.title} description={module.description}>
    <div className="mx-auto flex max-w-6xl flex-col gap-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{module.items.map((item, index) => <Card key={item} className="transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-sm"><CardHeader className="gap-4"><div className="flex items-center justify-between"><div className="flex size-10 items-center justify-center rounded-lg bg-primary/10 text-primary"><Icon /></div><Badge variant={index === 0 ? "default" : "outline"}>{index === 0 ? "Ativo" : "Em estruturação"}</Badge></div><CardTitle className="text-sm">{item}</CardTitle></CardHeader><CardContent className="flex items-center justify-between border-t pt-4 text-xs text-muted-foreground"><span>{index === 0 ? "Pronto para uso" : "Aguardando configuração"}</span><ArrowRight className="size-3.5" /></CardContent></Card>)}</div>
