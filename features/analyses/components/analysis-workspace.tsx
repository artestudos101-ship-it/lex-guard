...
import { ArrowLeft, BookOpen, ChevronRight, Eye, FileText, Filter, Layers3, Package, Search, ShieldCheck, SlidersHorizontal } from "lucide-react"
  ...
  return <AppShell title="Analysis Workspace" description={`${analysis.id} · processo observável`}>
    <div className="flex h-[calc(100vh-7rem)] min-h-[680px] flex-col overflow-hidden rounded-xl border bg-card shadow-sm">
      ...
      <div className={`grid min-h-0 flex-1 ${focus === "Documento" ? "lg:grid-cols-[1.8fr_1fr_.6fr]" : focus === "Decisão" ? "lg:grid-cols-[.7fr_1.1fr_1.3fr]" : "lg:grid-cols-[1.1fr_1fr_.65fr]"}`}>
        ...
        <aside className="min-h-0 bg-muted/10"><ScrollArea className="h-full"><div className="p-4"><DecisionContext analysis={analysis} /><Card className="mt-4"><CardHeader><CardTitle className="text-sm">Contexto vinculado</CardTitle></CardHeader><CardContent className="space-y-3 text-xs text-muted-foreground"><div className="flex items-center justify-between"><span>Documentos</span><span className="font-medium text-foreground">{analysis.documentNames.length}</span></div><div className="flex items-center justify-between"><span>Regras</span><span className="font-medium text-foreground">3</span></div><div className="flex items-center justify-between"><span>Versão da política</span><span className="font-medium text-foreground">v3.2</span></div><Separator /><p>Configuração controlada pelo administrador. Alterações no conjunto de documentos e regras não fazem parte da análise em execução.</p></CardContent></Card></div></ScrollArea></aside>
      </div>
    </div>
  </AppShell>
}
